...
from coopihc import Bundle, TrainGym, WrapperReferencer
...
config_task = dict(gridsize=20, number_of_targets=19, mode="position")
...
def make_env(seed):
     ...
     # Use env_checker from stable_baselines3 to verify that the env
adheres to the Gym API
     check_env(env)
     return env
...
# big_coopihc_expert = env.unwrapped.bundle.assistant
# _action, _ = big_coopihc_expert.predict(obs, deterministic=True,
wrappers=True)
...
def main():
     ...
     # reward, _ = evaluate_policy(expert.policy, Monitor(env),
n_eval_episodes=50)
     # print(f"Reward expert before training: {reward}")

     # print("Training the expert...")
     # expert.learn(10000)  # Note: set to 100000 to train a proficient
expert

     big_coopihc_expert = env.unwrapped.bundle.assistant

     reward, _ = evaluate_policy(big_coopihc_expert, Monitor(env),
n_eval_episodes=50)
     ...
     reward, _ = evaluate_policy(novice.policy, Monitor(env),
n_eval_episodes=50)
     print(f"Reward novice before training: {reward}")

     dagger_trainer = DAgger(env=env, expert=big_coopihc_expert,
policy=novice.policy)
